...
def quantize_model(ckpt_path: Path, quant_ckpt_path: Path):
    ...
    weights = l4jax.load_pytree(ckpt_path, l4jax.Weights.shardings(dataclasses.replace(cfg, quant_moe=False, quant_mlp=False, quant_attn=False)))

    print("Converting weights...")
    quant_layers = [l4jax.Layer.quantize(layer, cfg) for layer in tqdm(weights.layers, total=len(weights.layers))]
...
def split_gate_up_proj(key: str, weight: torch.Tensor, cfg: l4jax.Config):
    match = re.match(r"language_model\.model\.layers\.(\d+)\.feed_forward\.experts\.gate_up_proj", key)
    layer_idx = int(match.group(1))
    assert weight.shape == (cfg.moe_num_experts, cfg.embed, 2 * cfg.moe_ffw_size)
    return {
        f"language_model.model.layers.{layer_idx}.feed_forward.experts.gate_proj": weight[..., :cfg.moe_ffw_size],
        f"language_model.model.layers.{layer_idx}.feed_forward.experts.up_proj": weight[..., cfg.moe_ffw_size:],
    }
...
def convert_model_or_layer(
    ...
    torch_params = dict(llama_layer.named_parameters() if hasattr(llama_layer, "named_parameters") else llama_layer)
