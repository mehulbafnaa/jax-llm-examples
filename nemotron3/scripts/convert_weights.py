...
def main(model_path: str | Path, ckpt_path: str | Path):
    ...
    weights = n3jax.Weights.abstract(dataclasses.replace(cfg, quant_moe=False, quant_mlp=False, quant_attn=False))
    ...
    if not ckpt_path.exists():
        ...
        converted_weights = utils.convert_model_or_layer(weights, model, cfg, sequential=True)
        n3jax.save_pytree(converted_weights, ckpt_path)

    # < 100 MB or so
    for full_path in [path for path in model_path.glob("*") if path.stat().st_size < 100e6 and path.is_file()]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source-path", default="~/Nemotron3-30B-A3B-Nano", required=True, help="HF model directory path"
    )
    parser.add_argument(
        "--dest-path",
