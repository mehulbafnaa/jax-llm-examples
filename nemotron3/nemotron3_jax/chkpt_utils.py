...
def _stack_experts(params: dict[str, torch.Tensor]):
  ...
  for kw in ["up", "down"]:
    ...
    prefix_groups = set([re.match(match, k).group(1) for k in keys])
    for group_prefix in prefix_groups:
      keys_to_merge = list(sorted([k for k in keys if k.startswith(group_prefix)], key=key_fn))
      for k in keys_to_merge:
        del new_params[k]
      suffix = re.match(match, keys_to_merge[0]).group(3)
      new_params[f"{group_prefix}experts.{kw}_proj.{suffix}"] = torch.stack([params[k] for k in keys_to_merge], 0)
  return new_params
