...
def main(model_path: str | Path, ckpt_path: str | Path):
    ...
    assert config_file.exists(), "Must have only one `config.json` file in the model path"
    config = AutoConfig.from_pretrained(config_file)
    cfg = gpt_jax.hf_to_jax_config(config)

    # we convert the model unquantized when reading a GPT OSS model checkpoint
    weights = gpt_jax.Weights.abstract(dataclasses.replace(cfg, quant_moe=False, quant_attn=False))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source-path", default="~/gpt-oss-20b", required=True, help="HF model directory path")
    parser.add_argument(
        "--dest-path",
