...
class TestModel(parameterized.TestCase):
    def setUp(self):
        ...
        self.small_maverick_cfg = dataclasses.replace(
            MAVERICK_CFG, mesh=self.mesh, num_layers=4, embed=64, moe_num_experts=4, vocab_size=128
        )
    ...
    def test_model_init(self, scout, quant):
        ...
        cfg = dataclasses.replace(cfg, quant_attn=quant, quant_mlp=quant, quant_moe=quant)
