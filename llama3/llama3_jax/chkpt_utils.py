...
def quantize_model(ckpt_path: Path, quant_ckpt_path: Path):
    ...
    weights = l3jax.load_pytree(ckpt_path, l3jax.Weights.shardings(dataclasses.replace(cfg, quant_layer=False)))

    print("Converting weights...")
    quant_layers = [l3jax.Layer.quantize(layer, cfg) for layer in tqdm(weights.layers, total=len(weights.layers))]
...
def _llama_key_to_jax_key(llama_key, custom_key_map: dict[str, str] | None = None):
    key_maps = [_MODEL_KEY_MAPPING, _LAYER_KEY_MAPPING] + ([] if custom_key_map is None else [custom_key_map])
